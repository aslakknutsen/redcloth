module RedCloth::Formatters::ASCIIDOC
  include RedCloth::Formatters::Base
  
  [:h1, :h2, :h3, :h4, :h5, :h6].each do |m|
    define_method(m) do |opts|
      "\n#{''.ljust(m.to_s[1].to_i, '=')} #{opts[:text]}\n\n"
    end
  end
  ...
  def li_open(opts)
    "#{(@in_ordered_list ? '.':'*') * opts[:nest]}#{pba(opts)} #{opts[:text]}\n"
  end
  ...
  def table_open(opts)
    @in_table_row_count = 0
    @in_table_colum_count = 0
  ...
  def tr_open(opts)
    #\t<tr#{pba(opts)}>\n"
    @in_table_row_count += 1

    ""
  end
  
  def td(opts)
    @in_table_header = opts[:th]
    @in_table_colum_count += 1
    #\t\t<#{tdtype}#{pba(opts)}>#{opts[:text]}</#{tdtype}>\n"
    td_data = "|#{opts[:text]}\n"
    if @in_table_row_count == 1
      @table_buffer += td_data
      ""
    else
      td_data
    end
  ...
  def tr_close(opts)
    #{}"\t</tr>\n"
    if @in_table_row_count == 1
      header = "[cols=\"#{@in_table_colum_count}\""
      header += ", options=\"header\"" if @in_table_header
      header += "]\n"
      header + @table_buffer + "\n"
    else
      "\n"
